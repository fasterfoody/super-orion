...
import logoSvg from '@/assets/diyibu.jpg';
...
export function Sidebar() {
  ...
  useEffect(() => {
    // DISABLED: fetchAgents called by Dashboard instead
  ...
  const navItems = [
    { to: '/', icon: <LayoutDashboard className="h-[18px] w-[18px]" strokeWidth={2} />, label: t('sidebar.dashboard') || 'Dashboard', testId: 'sidebar-nav-dashboard' },
      ...
      <div className={cn("flex items-center p-2 h-12", sidebarCollapsed ? "justify-center" : "justify-between")}>
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <div className="flex items-center gap-1.5 px-1.5 py-1 rounded-md hover:bg-black/5 dark:hover:bg-white/5 cursor-default">
              <span className={cn(
                'w-2 h-2 rounded-full shrink-0',
                gatewayStatus.state === 'running' ? 'bg-green-400 shadow-sm shadow-green-400/50' :
                gatewayStatus.state === 'starting' || gatewayStatus.state === 'reconnecting' ? 'bg-amber-400 animate-pulse' :
                gatewayStatus.state === 'error' ? 'bg-red-400' :
                'bg-gray-300 dark:bg-gray-600'
              )} />
              {!sidebarCollapsed && (
                <span className="text-[11px] text-muted-foreground font-medium">Gateway</span>
              )}
            </div>
          </TooltipTrigger>
          <TooltipContent side="bottom" className="text-xs">
            <span className={gatewayStatus.state === 'running' ? 'text-green-400' :
              gatewayStatus.state === 'error' ? 'text-red-400' : 'text-amber-400'}>
              {gatewayStatus.state === 'running' ? '✓' : gatewayStatus.state === 'error' ? '✗' : '○'}
            </span>
            {' '}
            Gateway
            {gatewayStatus.state === 'running' && gatewayStatus.port && ` · port ${gatewayStatus.port}`}
