...
function WelcomeScreen({ onQuickAction }: { onQuickAction: (text: string) => void }) {
  const { t } = useTranslation('chat');
  const quickActions = [
    { key: 'askQuestions', label: t('welcome.askQuestions'), prompt: 'I have some questions I\'d like to ask you.' },
    { key: 'creativeTasks', label: t('welcome.creativeTasks'), prompt: 'Help me with a creative task.' },
    { key: 'brainstorming', label: t('welcome.brainstorming'), prompt: 'Let\'s brainstorm some ideas together.' },
  ];
      ...
      <div className="flex flex-wrap items-center justify-center gap-2.5 max-w-lg w-full">
        {quickActions.map(({ key, label, prompt }) => (
          <button
            ...
            className="px-4 py-1.5 rounded-full border border-black/10 dark:border-white/10 text-[13px] font-medium text-foreground/70 hover:bg-black/5 dark:hover:bg-white/5 hover:text-foreground transition-colors bg-black/[0.02] cursor-pointer"
          >
            {label}
          </button>
        ))}
