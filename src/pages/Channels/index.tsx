...
export function Channels() {
  ...
  const fetchPageData = useCallback(async () => {
    ...
    setError(null);
    try {
      // Fetch agents first to get channelAccountOwners mapping
      const agentsSnapshot = await hostApiFetch<AgentsSnapshot>('/api/agents');
      setAgents(agentsSnapshot.agents || []);

      // Fetch channels via renderer's fast WS connection, passing channelAccountOwners for agent binding
      const channelGroupsData = await useChannelsStore.getState().fetchChannelGroups(agentsSnapshot.channelAccountOwners);
